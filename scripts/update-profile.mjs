#!/usr/bin/env node
// Refreshes the `projects` and `articles` arrays in src/data/profile.json from
// the GitHub API (top repos by stars) and this blog's own RSS feed. Everything
// else in profile.json is hand-authored and left untouched.
import { readFile, writeFile } from 'node:fs/promises';

const USER = process.env.GITHUB_USER || 'duthaho';
const BLOG_URL = process.env.BLOG_URL || 'https://duthaho.dev/';
const FEED_URL = process.env.FEED_URL || new URL('rss.xml', BLOG_URL).href;
const TOKEN = process.env.GITHUB_TOKEN;
const DATA_PATH = new URL('../src/data/profile.json', import.meta.url);
...
function findArrayRange(src, key) {
  ...
  if (!m) throw new Error(`Key "${key}" not found in profile.json`);
